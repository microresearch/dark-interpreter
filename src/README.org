...
- test new wormdir
- tune speed of fingers somehow
- need set foldback settings in mainmode (add in)
- fill out mainmodes
- third ATTACH knob is underused (should always be attached)// added into mirror code

/////

- redo IFDEFs last(what hardware could be replaced by)?, possibly also settings.h as IFDEFed for LACH!!!
- test LACH and other board (first with audio.c effects)
