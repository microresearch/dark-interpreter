...
re-routing, and feedback. Order with 9v socket (not recommended for
safety reasons) or battery clip (default) on the back.
...
test 8,9,10 in audio, then

- option for dictated direction of playback (complexity setting)DONE-TEST!->5

- re-try leakage etc. code in cpuint

- double up audio switches with playback all of datagen

- how we handle edger? via complexity? as set of options

- audio.c sample wrap size/constraint=grainsize as part of speed setting perhaps

- new hardware settings:

- that main.c hardware dir can also be wormdir>hardware option?

in hardware.c maybe enlarge range res2 to 32 (combinations of
  hangs/unhangs, filter feeds and so on)

add flag for filtermix back in to audio (digfilterflag=32) and TEST

///////////

*complexity*:

audio: 0-> for granulation +[ effects, digfiltereffects=settings]

edger cases?

main: [running of 4 datagens(order of running_STACK?)]still question
of stack/push/pull as knob
...
main.c:
  ...
  lmer->step=1; lmer->speed=1; 
  maximer->step=1; maximer->speed=1; 
  f0106er->step=1; f0106er->speed=1; 
  hdgener->step=1; hdgener->speed=1; 
...
but we should have speedwrappers for cpu and leak?
...
- further tweaks/TODOs each file - WED/THU
