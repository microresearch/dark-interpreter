...
- reduce and expand all stack sizes as a mode:

CPU: ->m_threadcount(max MAX_THREADS)
CA: stack_posy (max STACK_SIZE)
sim: stack_pos (max STACK_SIZE)
villagers: villagestackpos (in twos)(max VILLAGE_SIZE)

// select stack (left/right) (knob and set)

- which modes work best// trim modes!
