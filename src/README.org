...
- improvements to audio.c walkers - see 427 - count now as signedTESTall walker settings
- volume for feedbackTEST in codec.c

- check villager max number.... in main init add all datagens _and_ villagers. TODO>TEST
- check datagens (maybe move push/pull earlier)
- full tests for each knob and fix settings X and Y somehow

- check all TESTYs and TESTERs, cleanups
- tweaks as below//FINAL TESTS/tests for simple boards
...
16 or keep as 128 as LIMIT?TODO?
...
- if we use 40106 as control then should do pwm on this all the time???
