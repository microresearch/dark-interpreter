...
- sketch roughly out whole thing then maybe return to effects and
  effect walker as maybe change model there???

=overlapsDONE/leftwalkersDONE/HW walkersTEST_TUE4NOV/(datagen walker) = mirrors/infection 4NOV// effects and effect walker finally

// test.c sanity AND test all WED05 *try and finish sketch*

- what are difference TENE?SUSP (in HW) so can also have 2 makefiles just for 1/LACH or 2/SUSP+TENE

- specify effects: vocoder/spectral=FFT/genericfilters/vowelcascadeBPFs/envelope&windows/morph&loop/convolution

- need to initialise _all_ villagers and mirror villagers for later swaps/effects etc.

- decide how mirror villagers work (is it just for offset.start and wrap and how mirrors modulate?)

TO RESOLVE: effects processing, datagen walker, mirroring

*** DONE

**** NOV 4

- started on mirror structure and effects to start to fill out!
...
- remember to initialise pos and all rest in audio.c!

- speeds still to do in effects walker
...
/////////////////////////
		...
		splitFilt = BPF.ar(sig, centerFreqs + MouseX.kr(10, 300), 0.03); 
...
- left/filter channel: left into audio_buffe, out from villagers=audio DONE
...
- defines for HW versionings or seperate makefiles!
