...
- granular tests=EXPAND>CONTRACT
- think to add new datagens (from sci-am)-EVE

** NOTES: 

randi as one issue (as varies quite small)

SIMULATIONS to fix: SEIR, IFSY, secondrossler,brussels,spruce,oregon?
...
- exeperms doesn't include non execution of any
- stacks or no stacks for execution (more control?)
- walkers can also be like ants, turmites and so on - MODES!
...
or we need 3 sorts of villager for each mode R?W?F?

  - is stack model working?
...
- re-familiarize and make general tests - cpu in CA and SIM should be
  alterable in stacker and stackery
- small fixes to effects
- expand AND contract the WRAPS _ TODO in tests
