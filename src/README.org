...
differing bandpasses (LIST) with frequency tables
convolution -DONE
phase/FFT vocoding/spectral
diff bandpass vocoder tests (mdaVocoder.cpp)
envelope/windowing

- overlap/effect in audio.c now with parameter? - like FMOD?

but need to define effects/overlays - do we split on datagen swop or
is that just one effect?

4. basic working model with overlap/left-filter/HW/datagens/mirrors
5. integration all buf16/audio/villagers
/////
...
-mirrors: mirror villagers as buffers, mirror villager parameters->datagen/eeg/knob/fingers,fingers in code
...
- simple per sample effects/overlap and effects walker
...
- effects somehow decided as specific chunk
