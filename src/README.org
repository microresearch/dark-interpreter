...
to init once or for each setting and share.. rethink BPF and Formlet cascade and vowel - also add in amp!

NOT cascade but in parallel?

////////if (freq != unit->m_freq || bw != unit->m_bw) {

discard:BBandpass???, biquad.c

--one FFT in pv (now with 32 samples)
