...
- TEST mirror/setting mode code (in full)
- tune speed of fingers somehow - fractional 0.1 increments (float? - test first in test.c)

- need set foldback settings in mainmode (add in)
- fill out mainmodes
//- third ATTACH knob is underused (should always be attached)// added into mirror codeDONE
