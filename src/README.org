...
so: TODO is effects walker, test datagen walker, mirrors/infections
and fill out all: again:::

- readin/out reworked DONE NOV 18
- filtin/out handling and digfilter flag!
- effects filled in/test
- mirrors filled in/test

handle? readin: in audio//left?/datagen/   ---buffer(overlap)

except LACH maybe keep left always open and used in HW???? reduce options...
...
constrain (without mirror) as also overlap makes no sense
...
also speedups in all - no division, no fmods!

- clean up how we set things  now without these!!!
