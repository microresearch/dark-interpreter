...
- filter and effects as walkers (add to settingsarray means we have > 32)DONE. TO TEST!
- how to test filter/leftbuffer effects?
- re-test hang/unhang in hardware

start on settings code etc. see below...

* SUN 20 APR


- TODO: do we want to use rdst more in audio.c // monobuffer???? we use it for leftbuffer only???
// so can discard it when we don't use leftbuffer

- TODO: look into ssat/usat to replace %32768 ???

- TODO: that wraps for sim, ca should always be constrained to 32768

- good to have knob for settings which could replace pull/pop knob (tho
  makes no sense to mirror this knob) or find somehow way to do this
...
*cpuintrev2.c:* as above; leakiness and co speeds;steering in wormcodeTEST
...
- redefine extra knob for LACH (as was edger!): - for LACH replace
  ...
  now)
- tweaking
- test LACH board only, parts count, recordings/video/documentation
...
- macro-chunk operations/feedback like this above???TODO
