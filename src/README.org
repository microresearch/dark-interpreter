...
- make clean after changes to settingsarray
...
The Dark Interpreter is _not_ to be controlled. It is a dark
electronic mirror, the skin itself.
...
4 and 5. 4 selects what parameter to set and 5 what the setting is
...
- clean vars/check arrays//

: main audioX simulationforstackX

//////

- check sim - rosllers and co for output scalings!
- try 48k again!
- fixes to fingervalright so wraps correctlyDONE-TEST!
- rework X and Y as new knobs: possibilities. X as mod. Y as settings
  for that group // open up bare fingers here and adc9
- but still need to access settings somehow... - what are key settings?
- masque code in pureleak and cpu TEST!
- test fingers with different fingers!
- FINAL TESTS/tests for simple boards
- finish write knob layout above for manual
- video and more samples...

- villager max number increase or?
- adc9 -> datagenbuffer (at start or?)TEST
- *some kind of foldback for hardware setting* settingsarray[64]//too high or?TEST
...
-clean up variables (ints, u16s esp. for dirs)
-audio.c simulation

-test behaviour on divide by zero

-estimate RAM use by way of arrays/structures (SEIR in sim is quite heavy max=1k)

-check null on inits in CA and sim???

-redo CA, CPU and sim without any mallocs... test without these
(CPU just mallocs at beginning)
