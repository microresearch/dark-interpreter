...
- importance of datagens for all so need fingers_in_all as last mode (well now is hardware)

- sketch roughly out whole thing then maybe return to effects and
  effect walker as maybe change model there???

=overlapsDONE/leftwalkersDONE/HW walkersTEST_TUE4NOV/(datagen walker) = mirrors/infection by 4th...
