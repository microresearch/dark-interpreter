...
- break down to arch. specific tests and do these. test lower potis,
  test Nan, malloc, simple board, all hardware, sampling rates change
  and so on

- further non-arch code

- fix cpuint issues below -> CPUintrev2.c FIXED but messy and test all
  CPUs from 6up, move ifndefs

- finish porting all to pureleak.c

- check for randi()%65536
...
- fixed m_pc bug in CPUint.c

- TODO 8bit bug in CPUint.c with machine poke always to 8 bit address

and in peek return u16 (preferred) or add start - fix also in pureleak

but need >>8 when we want 8 bit result!!!
