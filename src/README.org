...
- should have datagen->change all hardware switch code!

- add to knobs if have space - general complexity, from 0[signal
  thru/1maintain output/no write] to 3[signal thru/4datagens->hardware]
  to 5[change sample by datagen buffer/6hardware] to 7[basic granulation
  buffer/8hardware] to 9[granulation buffer as pointer to
  locations/10-hardware] to 11[+change sample/12-hardware] to
  13[worming thru buffer(according to datagen)/14change
  sample/15hardware] .. hardware as bit/mod...

or have datagens->hardware on hardware knob???
...
- edger should be direct poti(as all should be)! mark all as **TODO
...
- first attempt at audio buffer (how to restrict to 32768 index & 32768)
