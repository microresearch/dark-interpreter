...
- new layout/knobs basic tests: fingerUP as setting but will need to
  ...
  set ALL knobs as fixed or some and how some)

- fingers navigate audio (up/down/left/right as audio and datadirs now)
...
- what of untied fingers?

- add new datagens (from sci-am etc.)-worms through all, infectuous
  groupings and processes in settings

- per villager effects (how to attach) but no process attach (regions
  only)... how many villagers we can have in memory???

- refine execution and exespot

- randi as one issue (as varies quite small)TODO

- hardware is all same walker with shifts from base value!TODO!
