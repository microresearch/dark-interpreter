...
- re-test hang/unhang
- for LACH replace hardware knobs with 2x start.edge read.write (mirror divides into 4 now)

- bulk settings at angles -process across all settings eg.++(but
that is done by datagen walker)
...
- also that if flag is so can add left to possible audiobuffer
...
- how/if to work with larger chunk movements???
...
- redefine extra knob for LACH (as was edger!)
- tweaking
