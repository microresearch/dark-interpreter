...
- finish testing all modes and decide on any changes

and:

- stay on 48k
- fixed LM bug in hardware.c (did this effect anything? - NO)
- looking at 40106 range and PWM there//>>4 now so is 0->2048 range
...
- removed clock unhang in hardware.c as makes no sense
...
// test cases 4 and 5 and attachments, re-check audio.c/TEST!
// do we %5 for exestack so can be no execution?

later:

// samplerate variable on startup (8k,16k,32k,48k) knob0 TODO
// add depth/divider for maxim in hardware options (where?) TODO
// that can change its own setting layout/interface somehow - OVERLAY/MIRROR TODO
// more control over stack TODO
// expansion of wormcode TODO
 
- DONE:

// some kind of bufsel switch (top bit is not great) so operates more
on audio_buffer from CA and sim (also swaps there but these are
infrequent - now set so no bufsel!)- now as bufsel on lowest bit DONE

// to avg hardware knob or not- seems preferable DONE
