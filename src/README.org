...
- effects // but how much memory if we have say vocode instance for
  each villager
...
which FFT to use?, window should be const (as is size then of FFT)

pitchscale

SCcode=PV_SpectralMap (and all PV uses FFT from SC)

borsboom model- flow only or re-assess

OCT30: so far using arm_cfft_q15 test code...

3- band vocoders: how do we swap bands in our vocode.c and mda TODO/TEST!

(JoshUGens/sc/classes/Vocoder.sc is bandpass vocoder using BPF)

4- *envelope/windowing/hanning* - see also teensy, window.c and window in mdavocal.c

5- *envelope detection - test by enveloping another signal*

6- other effects once we have effect walker in place: morph one to
size of other, loop one within other...

recheck mda-lva, sc for any other ports
...
- overlap/effect in audio.c now with parameter like FMOD - IN
  PROCESS?TODO?TEST or have double inout buffers for float
...
- mirror as adding to or constraining what we have already defined for
  say end/start etc (table)

- so divide as to mirror walker struct as another(pointer) and to bend/constrain/influence another
