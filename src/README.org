...
* FRI-SAT 13 MAR
  ...
  wrap... DONE.TEST(simthenreal)REALTOTEST

- finish/test grain code - also normaldir TODO!

///

- test that datagens _do_run - listen

RETEST all - simu_CRASH!
...
- what relations could be between post-filter and input (we can go
  ...
  left INTO right)... but where to spec- as hardware settings are
  full

TODO: add as extra 2 options in hardware.c with control by what????

bitwise addition somewhere???
...
????set via walker for effects//complexity????-multiplies above
