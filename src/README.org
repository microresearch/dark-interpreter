...
The preliminary working of The Dark Interpreter (a dark mirror of the
reader, somehow reflecting the state of the improviser) should be
considered as part of the creation of a unique, skin-sensitive
instrument. The border between this first stage and the improvisation
and playing of the instrument is diffuse, the mirror surface
(obsidian) is ill defined.
...
simplify -> groups (maybe lose fingers), mirror and attach!

attach works well!

TODO:

- swop/dump settings/stacks to datagen in one mode
- per villager effects to test (in one write only now) TO copy across and TEST
- case 4 needs non-hardware ifdef/group
- case 5 has no knob/group interaction
- case 7-15 fingers need knob settings
- need to check toggling in wider context (full test)
- finish case 26
- wormdir and handdir also (put in one of finger modes)
- size of execstack
...
- having to tweak finger settings right down...TODO
...
- added audio wormdir code in WRITE only but still to toggle SOME WHERE!
...
- villagers: how many can we have?
- what knob settings in fingersmode could be???
