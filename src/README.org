...
- filter and effects as walkers (add to settingsarray means we have > 32)

- how to test filter/leftbuffer effects?

- do we want to use rdst more in audio.c // monobuffer???? we use it for leftbuffer only???
// so can discard it when we don't use leftbuffer

- chunk movers in simulation
- re-test hang/unhang in hardware

- look into ssat/usat to replace %32768 ???

*TODO/ideas*: 

- good to have knob for settings which could replace pull/pop knob (tho
  makes no sense to mirror) or find somehow way to do this

/////

- TODO: when wrapper changes we need to redo direction array!!!

- done away with cons (and processing) but simply altering
  all wraps in settingsarray once (as action) *TODO*
...
- settingsarray changed to 16bits (if like keep portion as 8 bbit for dirs but...)DONE
...
[5/6days]MON/TUE
- push and pop/buffer+swop/datagen settings
- new constraints of all datagens and walkers(inc foldback to areas
  only) under micro-macro -> file under ACTIONS again such as
  mirroring, constraint, expansion, reduction, remapping
- mirror knob: cross-settings option(mirror etc.) 
- knob and finger code

- extras like grain spawn mode, mirroring of settings
- TODOs in each file (check all audio so far) - recheck all TODOs as
  some are new:
*CAforstack.c:* buffer constraint in SIR16init
*pureleak.c:* hardware tie-in for masque; SPL in corewars,; var readins
*cpuintrev2.c:* as above; leakiness and co speeds;steering in wormcode?

[extras]WED/THU/FRI

- excess settings - actions such as read/write mirroring which are one off operations

- redefine extra knob for LACH (as was edger!): - for LACH replace
  hardware knobs with 2x start.edge read.write (mirror divides into 4
  now)
...
- add start and wrap for all datagens
...
- revising audio.c! TODO TO TEST and fill out...

*new ideas:*

thus TODO: revising complexity in audio.c, walkers and so on!

some kind of compression of data spaces (so we could >>bitshift
wrap/edge, restrict datagens to certain areas, also as is with cons
granularity, restrict foldback to areas - ie. expand and contract)

also ways of re-arranging larger chunks, potentially applying multiple
granularity as part of micro-macro (which could somehow replace
complexity knob - also how settings work with this - that we can globally
or locally apply certain settings)

restrict all walkers _and_ all datagens...

but need like simultaneous multiple levels of micro-macro

multiple repeated/stacked grain operations (reduce audio.c)- so is
like chunk of grains repeating NOT POSSIBLE or just do as mirrored
...
- test updates/tweaks in audio.c: wrap/edge in di_split_stereo

- where do we set cons granularity for audio.c?

[- question of villager metaphor/approach/micro-macro operations as complexity?
larger chunk contagion and movement?]

- TODOs in each file (check all audio so far) - recheck all TODOs as
  some are new

*audio.c:* where is cons set?; filter/leftbuffer effects; expand complexity/micro-macro thing
*main.c:* setup code for machines, sims etc;tune speeds and wraps; all knob code
*simulationforstack.c:* runknob assignment; question of start/end/step; bufferq
*CAforstack.c:* buffer constraint in SIR16init
*pureleak.c:* hardware tie-in for masque; SPL in corewars,; var readins
*cpuintrev2.c:* as above; leakiness and co speeds;steering in wormcode?

- steps in CA and simulation (fractional or?), maybe also end/start points for each 

- test all datagens and stack approach AGAIN
- swopping between audio and data buffers (should be done in
  stacks/+machine or as general/global setting?)

- knobs-mirror/settings and new settings
- fingers
- fill out extra complexity settings (and bring in more micro-macro cast)
- filter/leftbuffer effects/mirroring
- any other effects/morphs not covered by datagen swappery! 
- tweaks
