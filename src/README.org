...
=overlapsDONE/leftwalkers/hW walker/datagen walker and mirrors/infection by 4th...

- what are difference TENE?SUSP (in HW) so can also have 2 makefiles just for LACH or SUSP+TENE
...
- port from RW (given above?)  but how we handle if we have no in from
  ...
  seperate smaller left readin buffer
