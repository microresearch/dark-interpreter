...
- somehow to define complexity/interface in that first mode!
...
- started on speedups from below - most used as effect.c and audio.c (checking audio now for speedups)
- *re-instate howmany in datagens upto 32 so is not just one - but pos wrap needs be in each one or not all???* but where we can set this howmany???

CA/pureleak/CPU/sim

and also does pos go/should go less than zero!?

THU 26 and next week:

- TESTS! from WED
- sampling speed and final tests, TODO and tweaks/sanity checks 
- ifdefs for LACH and test LACH/SUSP (to build)

- speedups in main.c loops, effect.c code and audio.c (mostly)
...
- re-test all sections again: - re-test each section:
