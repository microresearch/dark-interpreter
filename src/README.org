...
- how do we apply per-villager effects now?
- speedup
- reduce max grains to 64

** DONE:
- TEST potis work (one to fix as hardware issue, otherwise top is
  4095, lowest is 1-8 say
...
- logarithmic scaling for start/wrap HOWTO? as lookup (4096 values as const)

generate with: (see log_gen.py)
