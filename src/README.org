h, eternity with outstretched wings, that broodest over the secret
...
WED 5: OPEN FIRE: FINISH first sketch to be filled out also!

///THU/FRI:

- swop and copy somehow confusing - bit clearer now! but needs be tested and filled out
- test basics with mainmode in main.c
- HW tests/re-check
...
to init once or for each setting and share.. rethink cascade!!!

--sets of fixed bandpasses= BPFSC, Formlet, discard:BBandpass, biquad.c - choose one or two with set of parameters init
--one FFT in pv (seperate out as needs 256 samples???)

but we should set PV->fftLen=256

[--mdavocXremove]
--mdavocoder/// or vocode.c (delete vocode.c as not really working)

as not to init:
--convolve
--envelope follower
--windower -- but is set for 256
--variable bandpass

////- effects cleaned up/init all effect structures, filter structures
