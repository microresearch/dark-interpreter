...
the 3 boards and how they differ and can be used

differences between the three synths

*** Mater Lachrymarum 

No analogue processing hardware so all settings which relate to
hardware are redundant. The first knob, for hardware routing and
setting, is now repurposed as a constraint on all
villagers/grains. Finger/skin controls are as on the Mater Suspiriorum.
...
out for flesh world most of the important digital and analogue signal
paths and settings. This last synth also adds the crude
EEG/micro-voltage fanged amplifier. All knob controls are the same for
both synths, although the layout is different. Finger controls are
also different.

** knobs from top down and left to right (diagram for each)
...
1 (was hard/now samplewrap)
         3
	 4
2	 5

1-The first knob strictly controls the hardware operations and signal
...
distortions. We also control feedback of input to output and use of
one finger pad as an input for other electrical impulses.

2-The second knob controls audio effects acting on the incoming audio,
the output audio signal and the digital filter (when selected). Thi8s
knob sets the effect parameters (type of effect, use of data buffer or
audio, village/grain orientation). Effects can also be placed under
the control of plague generation (see foldback/mirror below). The effect
parameters are applied to one of the three signals (in, out, filter)
according to the setting of the third knob and the directional finger
pads (see fingerpads below).

3-The third knob is a modifier which sets the behaviour of the four
finger pads in setting and unsetting various control parameters such
as effects:

effects
push and pop on the black stack of operations
directions of villagers
speed
constraint
foldback settings. about foldback/mirror

//REDO4 and 5. 4 selects what parameter to set and 5 what the setting is

** fingerpoints (2 diagrams)
...
- new layout - mode quadrant as 1-groups/2-finger/3-mirror(inc
  eeg)/4-attach=finger/process/detach/knob

- simulate whole in one space(double each op)
- knob settings in fingers (esp for stack settings)
...
- check STACKSTART etc. are inited okayyyy in main.c
- TODO:add EXPANDS to all audio.c
- how to make villagers a bit clearer (also per villager effects see below)
- clean up execution- WHAT to execute and where/how to add or remove?
- bounce stacks AND CPU to villagers...
...
- re-test CPUintrev3 (esp ants/turmites) UPTO 6//corewars FIXES
...
- new layout/knobs basic tests: fingerUP as setting but will need to
  re-work fingercode and knobs togetherness (also if fingers should
  set ALL knobs as fixed or only some and how only some)
...
- hardware is all same walker with shifts from base value!TODO! FIX!
  (but shift is also important)
...
- fingers into CPU and pureleak (how to choose execution - again more control)
- nearly all settings are for walkers ???
- exeperms doesn't include non execution of any (how many extra options?)
- stacks or no stacks for execution (more control?)
