...
- mirroring kind of works but need new knob scheme perhaps still!!

- knob scheme and mirroring (on paper)
- what settings and actions remain to be sorted?
- how to access arrays/stacks?

////
KNOB NOTES:

- hardware knob (in LACH replaced)
- mirror-constrain-expand-shift_start walkers/settings/datagens/stacks/foldback
- operations on walkers/settings/datagens/stacks/foldback=foldback

but like keep mirror knob (as settings/ops on settings)
...
(and then where is push/pop?)

///////

- stack ops - see below
- audio sample/any settings to be set from stacks (ca and sim)
  (step/start/wrap set by sim and ca stacks)

///more abstractThu/Fri
...
-what push and pull can also depend on/be triggered by

(hardware start/wrap can also be set to stacks)

mirror left/right is more about settings/ops on settings division/foldback-see main comments
