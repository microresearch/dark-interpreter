...
- jitter/smooth see below!
- stack see below
- knob and finger settings codeTEST
- mirror and TEST!
- foldback and TEST!


///more abstractWed/Thu/Fri
...
- tweaking
- test LACH board only, parts count, recordings/video/manual
...
-push/pull walk across all stack settings
-what push and pull can also depend on
-add extra village audio settings to be set from main.c (set by sim and ca stacks)

(hardware start/wrap can also be set to stacks but how do we know which stack is executed)

-exestack is somehow wrong as idea but works to re-order execution

mirror left/right is more settings/ops on settings division/foldback

////

- solution for mirror and hardware jitter/all jitter or keep hardware
  as one setting and redo knobs(where to put effects - have settings only by hand)
...
- solution for push/pop values and some kind of window into stack settings
...
TODO---- to open up stack settings, to be able to say match grains
start/wrap to audio start/wrap (in micro-macro)

** notes:

- where do we re-write direction arrays?
- where do we set villageread,write,filt?-> in micro-macro
- pureleak running needs buffer swap
- lack of transparency in push/pop settings// also unknown crash???
- restrict howmany for pushes
...
- TODO: we done away with cons (and processing) but simply altering
  all wraps in settingsarray once (as action) 
