...
- finished extramirror and attach////-TO TEST+ test mirror toggling
  and running, attachments

- last mirroraction 31 as somehow this infection from selected
  point/area of selected system????

- villagers: how many can we have?, start/wrap/effect - per villager
  effects to test (as villageeff array)

- check that execution is working fully (examining datagen and note
  that pureleak gives us beginning bleeps as storage is there)

++from TUE/WED below

- knob settings in fingersmode ???
- tweak speed of fingervalright
- tweak speeds in settings.h (remember to update initialisation in main.c)
- wormdir!

- refine and add in IFDEFs in main.c
...
- changed mirrors so is not always << and >> using eff[x]
