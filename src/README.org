...
- boards started
- test and tweak
- test fingers with different fingers-re-test LACH and SUSP(bring) boards with new knob layout
- finish manual/site
- video and more samples uploaded

* WED 7 MAY

- redo simulationforstack as sans mem - simforstacksansmemDONE-TEST!


- FINISH way into stack of cpus and more execution stuff

max is  this->m_threadcount

TEST :  this->m_threads[n].CPU=xxx%31
...
[TODO:
- reduce always-running mirror ops???TEST
- order of mirroring/foldback(first) and settings(later)TEST
- more of EXEC settings - stack, also fingers in the stack(HOW?)
- fingermod doesn't seem work so well - re-test
- maybe retry with float mod
]
