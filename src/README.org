...
-HW and input
-villagers and compression
-effects
-left/filter channel: left into audio_buffer/buf16, out from villagers/audio/buf16
-mirrors: mirror villagers, mirror villager parameters->datagen/eeg/knob/fingers,fingers in code
...
bitselect/overlapselect - on second compression knob pair?

what else could be on that knob (incoming HW? effects??)

- decide on effects per sample (read and/or write and/or filter) or
  effects walker??? (and/or FFT walker)

- clearer scheme for buf16 read _and_ write out!

- global offset start - somehow fix compress/expand so is really doing so?

- nextdatagen() as function for datagen walker...

- villager-villager effects - make a clearer format as also need
  process each effect in chunks (and to keep track of these)
...
exchanges/4-simple formant vowels/5-further
...
2. 16 channel bandpass/formant/vocoder: ladspa or vst-mda version
...
(convulution = multiplication in freq domain)
...
formant: filter signals - (SC=formant.ar, BBandpass?, formlet? where?), biquad (single formant)

/root/supercollider/server/plugins -> BBandpass (complicated)
