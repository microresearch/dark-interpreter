...
- datagens: threading for instructions - see leaky.c

* FRI 27 DEC

- turmite and ant code added as CPUs to leaky.c (total now 13 CPUs)

- TODO add for datagens: CAs(one dim, life, two dim)DONE, finish
  simulations, SIR and hodge.

working on hodge: microbd version, old hodge.c version, just found version: http://hunter.apana.org.au/~cjb/Code/hodge.c

- piece into datagens and resolve huge buffer question (perhaps that
  simulation-style datagens write history consecutively into working
  buffer but reserving first part for settings)
