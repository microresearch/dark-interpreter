...
- what was slowdown in mutate etc/cpurev? in leak FIXED but still
  cpuint doesn't sound as good as on PC

[- question again of randi() as adc???]

6-test clocks and unhanging?->needs code to finish it - coded but
needs TESTING (whether re-hang is working with just re-attach...

reset_clocks function in hardware.c
...
9-cleanups/all TODOs in each file

-> into buffer/complexity/speed code/knobs
