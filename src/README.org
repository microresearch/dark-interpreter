...
- samplerate changes crash when we don't do STRAIGHT/ie. use hardware
  changes/sethardware or pwms???? narrow down

- cleanups/check all TODOs in each file//plan

quick knobs: 1hardware=ADC2, 2speed/samplerate(16*16)=ADC0, 3complexity,
4settingforup/down(inc add/remove stacks?), 5/edge(edge set as datagen or as
straight set)-or 5/add/remove stacks (with edge in up/down)

////

10-audiobuffer->grainstart/end
