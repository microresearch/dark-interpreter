...
hardware are redundant. The xxxxx knob, for hardware routing and
setting, is now repurposed as . Finger/skin controls are as on the
Mater Suspiriorum.
...
- tweaks
- recordings/video/manual/website
- test LACH and SUSP (first with audio.c effects)

* WED 18 JUNE

////IN STUDIO:

-measure M3 schrauben (order 6mm or??)

- tuning speed of fingers - fractional 0.1 increments - TEST with first case FIRST!
- TEST mirror/setting mode code (in full so far)
- is attach (3rd knob) underused? cases are: 9,16,25
- should we use UP/DOWN for foldd[1] in cases 26+

/////
EVENING:

- re-check CPU,pureleak,audio and clean UP!
- retest formant and other functions in sim

////

- redo IFDEFs last(hardware knob replaced with EFFECTREAD), also
  settings.h as IFDEFed for LACH!!!
...
- mismatch of bits in pushn in ca and simX
