...
GPIOC? FIXED - problem was extra GPIO_Init(GPIOC, &GPIO_InitStructure)
...
- functions (in sim) to write knob or eeg values into datagen buffer - DONE (nearly)
...
maybe add a delay around this for whole cpu????

*/// array could be settings we run through when we push to stack and so on...**
...
1-test lower potis(hardware), DONE - todo- test all of them...

problems with ccmdata as wasn't in flash.ld but if we use 407.ld or
add ccmdata to flash.ld we end up with huge binary???

how to test with pointer straight to memory? write&read datagen->audiobuffer
