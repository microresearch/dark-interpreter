...
- do and test hardware walkers with start/wrap and cons:
- cons for h walkers on wrap (speed of using cons as limit also with if and not %)

NOTETODO: - when wrapper changes we need to redo direction array!!!
