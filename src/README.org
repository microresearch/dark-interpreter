...
- mark all questions as **TODO

* DONE

- add ifdefs, includes etc. for PC/arm code compilation // fixing now
...
- overlap and uint16 sizes for CPU.c or somehow 2 sets options - now
  working in CPUint.cDONE - do we need two versions?


* WED 22 JAn 2014

rough poti list (top down= 2,0,3,4,1):

0-hardware(2) (and for Lachrymarum? - shift some modifiers

1-micro->macro with cross - how? (0) - mods the cross/so for example
code/sample steering enabled, add/subtract from stack..?

2-edge (for incoming samples) (3)

3-modifier (4) - eg. speed, selection, re-inits, add to stack, memory
exchange,samplerate, leakiness, mutation rate, sample effects,
granular or not, step through datagen buffer...
...
how micro-macro knobs works with this and how we add/subtract to cpu
stack...

option: grain stack//straight read samples from datagen position

also that we could have to/from limits for reading from datagen

path through datagen can be stepped, can be dictated by datagen... x
removed steps thru

// TODO:

- 16 bit math ops in cpuint - DONE and also fixed float error in hodge

- fix simulation.c overflow possibility - SHOULD be fixed - TEST!
...
- complexity can also figure how many simultaneous datagens

- also datagen rebooting!
