...
mirror surface (obsidian) is ill-defined. The Dark Interpreter leads a
path for skin, fingers, earth and head. 
...
differing). This first knob selects one of 32 modes, divided into
...
follows. Each mode effects the beviour of the finger-board and the
second and third knobs. All modes allow for entry into the core plague
code (fingers in the code), audio settings, villager parameters and
mirror settings. Behaviour is different for each set of modes (see
below).

Basic modes:

Basic modes allow for setting of specific input, audio, code, villager or
mirror settings primarily using the second knob (see below) and the
finger-board for setting and navigation/selection of
settings. Code/data can also be used to select settings under various
modes. 

We also control feedback of input to output and use of one finger pad
as an input for other electrical impulses.

Mirror modes:

Mirror modes allow for the attachment and recursive folding back of
code onto its own settings.

up=unset
down=set

2- This knob controls specific settings within the first 16 basic
modes (only when we have a finger on the DOWN pad of the
finger-board). In the case of the later 16 mirror modes, this knob
controls the extent of mirroring and in the last case (31) key plague
or infection-mirror parameters.

3- This third knob is only used when certain parameters are attached
to it by way of some of the mirror modes. Attachments can also be made
to code and finger-down pad.

4-The fourth knob controls audio effects acting on the output audio
signal. This knob sets the effect parameters (type of effect, use of
data buffer or audio, village/grain orientation and gathering). 

In the case of Suspiriorum and Tenebrarum the effect parameters are
applied to one of the three signals (in, out, and filter feedback) by
way of an offset from this main control (this offset can be set and
influenced by the mode setting).

In the case of Lachrymarum (no analogue hardware), this knob controls
the output audio effects _only_.
...
controls incoming audio effect parameters (as above).
...
681(noard,IN),673(nohard,TMin),674*(hard!,TMin),675(nohard,TMin),676(nohard,noin),677/678(hard,noin),680(noard/noin)

///////////////////////////
- check all files

- new LACH question on finalknob
- uses of minormode?
- test LACH and SUSP - full tests/tweaks each mode///manual///documentation///website


//////////////////- re-testing all modes!FIX-/-RE-TEST!
...
3- setted for all SETTED! - NOW as one mode (minormode as mod)
...
7-infection (could be across all?) INFECTION - size mismatch - infect across all
...
- EFFECTWRITE=adc_buffer[FOURTH]>>6 on modes 0/1/2
...
- effect offsets elsewhere than settingsarray (set in mode 0 but how also set filter offset)
- final convolve effects fixed
- fix mode issue/silence..speed issue-> working on/DONE-tweakTODO!
- check wrap on knob1-REMOVED
- why inp mode failed 0 was wrap
