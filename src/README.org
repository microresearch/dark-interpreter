...
//tweaks?- is attach (3rd knob) underused? cases are: 9,16,25
...
- TEST mirror/setting mode code (each case//in full)

/////
HOME EVENING:

- re-measure M3 schrauben (8mm and check against other distanzbolzen at home)//order reichelt
- re-check CPU,pureleak,audio and clean UP!
- re-test functions in sim
- check again all bit sizes in settings.h
