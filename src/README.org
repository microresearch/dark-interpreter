...
- test new audio walkers - seems okay

problem is somewhere in new walker code (R,W or???)

- test each mode in main: TESTED: 0/1/2/3/4/5+effects/6-datagenwalker/7/8//9=mirror 10+=hardware

- ifdefs for LACH-DONE and test LACH (to finish) SUSP should be as TENE but test when ordered!
- re-test full and sanity check all again
...
- int32_t in village_read for samplepos only(but needs be in all as swap samplepos), small fixes in audio.c
...
- inlining bandpass code in effects but still effects seem odd
