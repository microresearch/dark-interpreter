...
* new scheme for development notes

From 28 OCT 2014+: to avoid repetition of notes (TODO: note DONE with DATEstamp)

** General TODO: 
 
- check datagens run ok. eg in sim: ifs, globals test in sim etc.

sim/ca/cpu/leak

- importance of datagens for all so need fingers_in_all as last mode

- *buf16 should also be additive (or/and/+ etc)*
...
- should effects walker overlap. how to time it and keep memory of
  timings, data structs

- how to assign/control parameters of effects (with setting for
  effects param) model this clearly

- test all effects with simple audio I/O (see below)/nearlyDONE
...
- all data structures for effects (instantiated also)
...
4- windowing/hanning - see also teensy, window.c and window in mdavocal.c

5- envelope detection - test by enveloping another signal

6- other effects once we have effect walker in place: morph one to
size of other, loop one within other etc.. stretches....

8- recheck teensy,mda-lva, sc for any other ports
...
difference convolution and vocoder? pitch shift// csound: http://music.columbia.edu/pipermail/music-dsp/2000-March/037231.html

//or have double inout buffers for float

I am trying to implement a channel vocoder using the iOS Accelerate
vDSP FFT algorithms. I am having trouble figuring out how to treat the
DC component and Nyquist frequency.

The modulator and carrier signals are both float arrays of length
n. On each, I perform a forward FFT and am returned a frequency plot
(call it bin[]) of length n/2.

As per the vDSP specifications, bin[1] contains the first frequency
above 0Hz, bin[2] the second, etc... bin[0] contains the DC Component
in the real part and the Nyquist frequency (which would normally be in
bin[n/2]) in the imaginary part. vDSP essentially packs the frequency
plot into as little space as possible (the imaginary part for bin[0]
and bin[n/2] should always be zero before the packing).

I split the frequency plot for both carrier and modulator into k
bands. My goal is to multiply each frequency in carrier.band[x] by the
total magnitude of the frequencies in modulator.band[x]. Essentially,
increasing the intensity of those frequencies in the carrier that are
also present in the modulator.

So if n=8 and k=2, the second band for the modulator would contain
contain bin[2] and bin[3]. Simple enough to find the total magnitude,
simply sum the magnitudes of each bin (for example mag[2] = sqrt(
bin[2].real*bin[2]*real + bin[2].imag*bin[2]*imag )).

That works great for all bands except the first one, because the first
band contains the weird bin[0] with the DC component and Nyquist
frequency.
...
- finish and test all for RW

- overlap/effect in audio.c now with parameter like FMOD - IN
  PROCESS?TODO?TEST or have double inout buffers for float...all as floats
...
- port from RW (given above?)

*** DONE

** Hardware Walker

- how switch bit/overlap mode or simple
...
- say 8 or 16 hardware villagers
...
- nextdatagen() as function for datagen walker to serve others
...
- any math speedups inspired by teensy(sqrt etc....)
