...
TODO: add as options in hardware.c with control by complexity 

- simple co-eff filter additions in cpuDONE simulation.cTODO (8bit and 16bit)

- do lower potis, build simplest, images, test potis/straight thru on
  both, test datagens on ARM (NaN etc.)

- fix count problem in simulation.c - TO TEST
...
machine_run(m); (which can also be slowed/delayed with surroundings)
...
so sim and ca can be stacked as function pointers, machine_run is bitwise
within complexity setting
...
3-modifier (4) - re-inits, add to stack, memory exchange, cpu-swap
