...
TODO: test push/pop on its own, make more of execution, effectmod is a
bit oddFIXED, order of mirroring/foldback(first) and settings(later),
hardware in foldback as questionableDONE, test finger stuff ONLY, test 16k
again(speedup?)

- thus more EXEC settings - stack, also fingers in the stack
...
- fixed exespot error - check settingsarray in main
