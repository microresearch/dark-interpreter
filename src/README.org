...
- test hardware walkers with start/wrap and cons and consider new walker scheme
- hardware wlakers new direction code?

- re-test all audio/datagens and audio

- filter and effects

ideas: bulk settings at angles -process across all settings eg.++(but
that is done by datagen walker)

- do we handle wrap okay (as just hits top with no wrap - we could
  just start and then run as long as wrap is)????

eg. 
		  if (f0106erpos>wrapper) f0106erpos=0;
		  tmp=(F0106ERSTART+f0106erpos)%AUDIO_BUFSZ;

NOTE_ *TODO*: - when wrapper changes we need to redo direction array!!!
